...
const blank = { id: "", main_cat_id: "", purity_id: "", rate_date: today, rate_per_gram: "", status: "active" };
...
export default function RateMaster() {
  ...
  const filtered = useMemo(() => {
    ...
    return rates.filter(r => {
      const okSearch = (r.metal_name || "").toLowerCase().includes(q) || (r.purity_name || "").toLowerCase().includes(q) || (r.rate_date || "").includes(q);
  ...
  const save = async e => {
    e.preventDefault();
    if (!form.main_cat_id) return toast("error", "Metal select karo");
    if (!form.rate_per_gram || Number(form.rate_per_gram) <= 0) return toast("error", "Rate sahi daalo");
    setSaving(true);
    try {
      const res = await axios.post(`${API_URL}?action=save`, form);
      if (res.data.status === "success") { toast("success", res.data.message); setForm(blank); load(); }
  ...
  const edit = r => {
    setForm({ id: r.id, main_cat_id: r.main_cat_id || "", purity_id: r.purity_id || "", rate_date: r.rate_date || today, rate_per_gram: r.rate_per_gram || "", status: r.status || "active" });
    ...
    <div className="rate-grid">
      <section className="rate-card form-card"><h3>{form.id ? "Update Rate" : "Add Rate"}</h3>
        <form className="rate-form" onSubmit={save}>
          ...
          <div className="field-row"><div className="field-group"><label>Date *</label><input type="date" value={form.rate_date} onChange={e => setForm({ ...form, rate_date: e.target.value })}/></div><div className="field-group"><label>Status</label><select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}><option value="active">Active</option><option value="inactive">Inactive</option></select></div></div>
      ...
      <section className="rate-card list-card">
        ...
        <div className="table-wrap">{loading ? <div className="loader"><FaSpinner className="spin"/> Loading...</div> : <table className="rate-table"><thead><tr><th>#</th><th>Metal</th><th>Purity</th><th>Date</th><th>Rate</th><th>Status</th><th>Action</th></tr></thead><tbody>{current.length ? current.map((r, i) => <tr key={r.id}><td>{(page-1)*perPage+i+1}</td><td><b>{r.metal_name}</b></td><td>{r.purity_name || "Standard"}</td><td>{r.rate_date}</td><td className="amount">₹{money(r.rate_per_gram)}</td><td><span className={`status-pill ${r.status}`}>{r.status}</span></td><td><button className="edit-btn" onClick={() => edit(r)}><FaEdit/></button><button className="delete-btn" onClick={() => del(r.id)}><FaTrash/></button></td></tr>) : <tr><td colSpan="7" className="no-data">No rate found</td></tr>}</tbody></table>}</div>
