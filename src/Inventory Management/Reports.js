...
export default function Reports() {
  ...
  const renderTable = () => {
    ...
    if (tab === "cashbook") return <table><thead><tr><th>Date</th><th>Type</th><th>Category</th><th>Amount</th><th>Mode</th><th>Notes</th></tr></thead><tbody>{current.map(r=><tr key={r.id}><td>{r.date}</td><td><b className={r.type === "IN" ? "green" : "red"}>{r.type}</b></td><td>{r.category}</td><td>₹{money(r.amount)}</td><td>{r.payment_mode}</td><td>{r.description}</td></tr>)}</tbody></table>;
