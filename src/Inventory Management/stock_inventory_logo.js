...
export default function StockInventory() {
  ...
  const barcodeHtml = (item) => {
    ...
    return `
      <html><head><title>${barcodeText}</title>
      <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Devanagari:wght@700;900&family=Inter:wght@400;700;900&display=swap" rel="stylesheet">
      <style>
        @page{size:70mm 35mm;margin:3mm}*{box-sizing:border-box;margin:0;padding:0}
        body{font-family:Inter,Arial,sans-serif;background:#fff;padding:0}
        .tag{width:64mm;height:29mm;display:flex;overflow:hidden;border:1.2px solid #d4af37;border-radius:8px;background:linear-gradient(135deg,#1c1108,#3b230d,#1d1209);page-break-inside:avoid}
        .brand{width:20mm;background:radial-gradient(circle,#4b2b10,#1b1108);display:flex;flex-direction:column;justify-content:center;align-items:center;border-right:1px solid rgba(212,175,55,.4);padding:.6mm}
        .shop-title{font-family:'Noto Sans Devanagari',sans-serif;font-size:8px;font-weight:900;color:#ffd96b;text-align:center;line-height:1.05;margin-bottom:.7mm}
        .logo-wrap{width:13mm;height:13mm;border-radius:50%;overflow:hidden;border:1.2px solid #d4af37;background:#221208;display:flex;align-items:center;justify-content:center}.logo-icon{width:100%;height:100%;object-fit:cover;border-radius:50%}
        .metal{margin-top:.6mm;font-size:4.5px;font-weight:900;color:#f7d36a;text-align:center}
        .info{flex:1;padding:1.5mm 1.8mm;background:linear-gradient(135deg,#fff9f0,#f6e5c3);display:flex;flex-direction:column;justify-content:center;text-align:center}.product{font-size:7px;font-weight:900;color:#241507;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.weight{font-size:6px;font-weight:800;color:#6b4314;margin-top:.6mm}.barcode-area{margin-top:1mm;background:linear-gradient(135deg,#b5be5f,#4d2b11);border-radius:4px;padding:.5mm;border:1px solid rgba(212,175,55,.5)}.barcode-img{width:100%;max-width:40mm;height:9mm;background:#fff;border-radius:2px;object-fit:contain}.code{margin-top:.3mm;font-size:4.8px;line-height:1;font-family:monospace;font-weight:900;color:#f7d36a;letter-spacing:.2px}
      </style></head><body><div class="tag"><div class="brand"><div class="shop-title">श्रीजी ज्वेलर्स</div><div class="logo-wrap"><img class="logo-icon" src="${logoUrl}" /></div><div class="metal">${item.metal_name || "Jewellery"}</div></div><div class="info"><div class="product">${item.product_name || "Item"}</div><div class="weight">Wt: ${weight(item.net_weight)}g</div><div class="barcode-area"><img class="barcode-img" src="https://bwipjs-api.metafloor.com/?bcid=code128&text=${encodeURIComponent(barcodeText)}&scale=3&height=14"/><div class="code">${barcodeText}</div></div></div></div><script>window.onload=()=>setTimeout(()=>{window.print();window.close()},500)</script></body></html>`;
  };
